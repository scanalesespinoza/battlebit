 ...
 * @version $Id: showlists.php 6643 2011-06-08 09:10:21Z giangnd $
...
class ImageShowModelShowLists extends JModel 
{
	...
	function _buildQuery() 
	{
		...
		$query		= ' SELECT sl.*, COUNT(img.showlist_id) AS totalimage, c.configuration_title, ag.title AS access_level FROM #__imageshow_showlist AS sl'.
					  ' LEFT JOIN #__imageshow_images img ON sl.showlist_id = img.showlist_id 
					  	LEFT JOIN #__imageshow_configuration c ON sl.configuration_id = c.configuration_id LEFT JOIN #__viewlevels AS ag ON ag.id = sl.access'
					. $where
