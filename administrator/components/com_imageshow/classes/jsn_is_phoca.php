 ...
 * @version $Id: jsn_is_phoca.php 6648 2011-06-08 10:13:51Z giangnd $
	...
	function getSyncImages($showlistID, $limitEdition = true)
	{
		...
		if (count($result) > 0)
		{
			foreach ($result as $item)
			{
				...
				if (count($images) > 10 && $limitStatus == true && $limitEdition == true)
				{
					break;
				}
			}
		...
		if (is_array($images))
		{
			foreach ($images as $img)
			{
				if ($img->custom_data == 1)
				{
					...
					$img->original_title 		= $info[2];
					$img->original_description 	= $info[3];
					$img->original_link 		= JURI::root().'index.php?option=com_phocagallery&view=detail&catid='.$info[1].'&id='.$info[0];
				}
				else
				{
