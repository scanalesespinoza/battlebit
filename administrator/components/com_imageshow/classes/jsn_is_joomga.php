 ...
 * @version $Id: jsn_is_joomga.php 6648 2011-06-08 10:13:51Z giangnd $
	...
	function drawXMLCate($parentID, $items, $syncAlbum)
	{
		...
		foreach ($items as $item)
		{
			if ($parentID == $item->parent)
			{
				...
				$item->cid  = htmlspecialchars ($item->cid, ENT_QUOTES);
		...
		if (is_array($images))
		{
			foreach ($images as $img)
			{
				if ($img->custom_data == 1)
				{
					...
					$img->original_link 		= JURI::root().'index.php?option=com_joomgallery&func=detail&id='.$info[0];
				}
				else
				{
