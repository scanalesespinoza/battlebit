 ...
 * @version $Id: default_profiles.php 6520 2011-06-01 08:33:51Z trungnq $
			...
			<table class="adminlist" border="0">
				<thead>
					<tr>
						...
						<th width="5%" nowrap="nowrap"> <?php echo JText::_('MAINTENANCE_SOURCE_SOURCE_TYPE'); ?> </th>
						...
						<td align="center">
							<?php
								if($row->source_type == 1){
									echo JText::_('MAINTENANCE_SOURCE_FOLDER');
								}elseif ($row->source_type == 2){
									echo JText::_('MAINTENANCE_SOURCE_FLICKR');
								}elseif ($row->source_type == 3){
									echo JText::_('MAINTENANCE_SOURCE_PICASA');
								}
							?>
						</td>
						<td align="center" class="actionprofile"><?php if($row->source_type == 2){ ?>
							<a rel="{handler: 'iframe', size: {x: 500, y: 250}}" href="index.php?option=com_imageshow&controller=maintenance&type=editprofile&source_type=<?php echo $row->source_type; ?>&tmpl=component&configuration_id=<?php echo $row->configuration_id?>" class="action-edit modal" title="<?php echo JText::_('EDIT')?>"></a>
							<?php }elseif ($row->source_type == 3){?>
							<a rel="{handler: 'iframe', size: {x: 500, y: 170}}" href="index.php?option=com_imageshow&controller=maintenance&type=editprofile&source_type=<?php echo $row->source_type; ?>&tmpl=component&configuration_id=<?php echo $row->configuration_id?>" class="action-edit modal" title="<?php echo JText::_('EDIT')?>"></a>
							<?php } ?>
							<a rel="{handler: 'iframe', size: {x: 500, y: 300}}" href="index.php?option=com_imageshow&controller=showlist&task=element&tmpl=component&limit=0&configuration_id=<?php echo $row->configuration_id?>" class="action-delete modal" title="<?php echo JText::_('DELETE')?>"></a>
						</td>
					</tr>
					<?php
					$k = 1 - $k;
