 ...
 * @version $Id: jsn_is_flickr.php 7413 2011-07-20 10:54:57Z trungnq $
...
class JSNISFlickr{
	...
	function getValidation($username, $apiKey, $secretKey)
	{
		...
		$this->_errorCode = $service->getErrorCode();
		$this->_errorMsg  = $service->getErrorMsg();
	...
	function getAlbumsList()
	{
		...
		if (count($photoSets['photoset']))
		{
			...
			return "<node label='Not In set' isBranch='true' data='0'></node>";
		}
		...
		if (is_array($images))
		{
			foreach ($images as $img)
			{
				if ($img->custom_data == 1)
				{
					...
					$img->original_title 		= $info['title'];
					$img->original_description 	= $info['description'];
					$img->original_link			= $info['urls']['url'][0]['_content'];
				}
				else
				{
