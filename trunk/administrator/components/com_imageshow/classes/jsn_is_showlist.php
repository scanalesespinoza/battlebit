 ...
 * @version $Id: jsn_is_showlist.php 6648 2011-06-08 10:13:51Z giangnd $
...
class JSNISShowlist
{
	...
	{
		$query 	= 'SELECT showlist_id FROM #__imageshow_showlist WHERE configuration_id IN ('.$configIDs.')';
		$this->_db->setQuery( $query );
			...
			switch ($sourceType)
			{
				case 'flickr':
					...
					for ($i=0, $n=count( $data ); $i < $n; $i++)
					{
						...
						$imageDetailObj->{'title'} 			= ($overrideTitle == 1) ? $showlistInfo['showlist_title'] : $row->image_title;
						$imageDetailObj->{'description'} 	= ($overrideDesc == 1) ? $showlistInfo['description'] : ((!is_null($row->image_description)) ? $row->image_description:'');
						$imageDetailObj->{'link'} 			= ($overrideLink == 1) ? $showlistInfo['showlist_link'] : $row->image_link;
				...
				case 'joomga':
					...
					for ($i=0, $n=count( $data ); $i < $n; $i++)
					{
						...
						$imageDetailObj->{'title'} 			= ($overrideTitle == 1) ? $showlistInfo['showlist_title'] : (($row->image_title!='') ? $row->image_title:$originalPhotoInfoOfJoomga['imgtitle']);
						$imageDetailObj->{'description'} 	= ($overrideDesc == 1) ? $showlistInfo['description'] : (($row->image_description!='') ? strip_tags($row->image_description) : strip_tags($originalPhotoInfoOfJoomga['imgtext']));
						$imageDetailObj->{'link'} 			= ($overrideLink == 1) ? $showlistInfo['showlist_link'] : $row->image_link;
						$array_image[] 						= $imageDetailObj;
					}
				break;
				case 'phoca':
					...
					for ($i=0, $n=count( $data ); $i < $n; $i++)
					{
						...
						$imageDetailObj->{'thumbnail'} 		= $URL.$row->image_small;
						$imageDetailObj->{'image'} 			= $URL.$row->image_big;
						$imageDetailObj->{'title'} 			= ($overrideTitle == 1) ? $showlistInfo['showlist_title'] : (($row->image_title!='') ? $row->image_title : $originalPhotoInfoOfPhoca['title']);
						$imageDetailObj->{'description'} 	= ($overrideDesc == 1) ? $showlistInfo['description'] : (($row->image_description!='') ? strip_tags($row->image_description) : strip_tags($originalPhotoInfoOfPhoca['description']));
						$imageDetailObj->{'link'} 			= ($overrideLink == 1) ? $showlistInfo['showlist_link'] : $row->image_link;
						$array_image[] 						= $imageDetailObj;
					}
				break;
				...
				default:
					...
					$syncData 		= $objJSNFolder->getSyncImages($showlistID);
	...
	{
		$query 	= 'SELECT showlist_title, showlist_id  FROM #__imageshow_showlist ORDER BY date_modified DESC LIMIT '.(int)$limit;
		$this->_db->setQuery($query);
