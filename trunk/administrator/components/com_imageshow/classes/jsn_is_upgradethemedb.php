 ...
 * @version $Id: jsn_is_upgradethemedb.php 6677 2011-06-10 01:37:28Z giangnd $
 * @license GNU/GPL v2 http://www.gnu.org/licenses/gpl-2.0.html
 */
defined('_JEXEC') or die( 'Restricted access' );
include_once(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_imageshow'.DS.'classes'.DS.'jsn_js_dbutils.php');
...
class JSNISUpgradeThemeDB
{
	...
	function setObjJSNJSDBUtils()
	{
		$this->_objJSNJSDBUtils = new JSNJSDBUtils();
	}
	...
	{
		$versionsArray			= array();
		$filePath 				= JPATH_ROOT.DS.'plugins'.DS.$this->_group.DS.$this->_folder.DS.$this->_folder.DS.'helper'.DS.$this->_fileName;
									...
									for ($z = 0, $count2 = count($fields); $z < $count2; $z++)
									{
										...
										if (count($datas))
										{
											for ($x = 0, $count3 = count($datas); $x < $count3; $x++)
											{
												...
												$objData->oldvalue			= $dataAttributes['oldvalue'];
												$objData->newvalue			= $dataAttributes['newvalue'];
												$this->_itemChangedValues[$fieldAttributes['id']][]	= $objData;
											}
										}
									}
								}
	...
	function buildQueriesTableAdd($data, $dataChange, $tableRemoved)
	{
		...
		if(count($data))
		{
			foreach ($data as $key => $value)
			{
				...
				if(in_array($key, $tableRemoved) || $this->_objJSNJSDBUtils->isExistTable($key)) continue;
	...
	function buildQueriesTableRemove($data)
	{
		...
		if(count($data))
		{
			foreach ($data as $value)
			{
				if($this->_objJSNJSDBUtils->isExistTable($value))
				{
					...
					foreach ($tables as $fields)
					{
						...
						if(count($fields))
						{
							for ($i = 0, $count = count($fields); $i < $count; $i++)
							{
								...
								elseif ($this->_objJSNJSDBUtils->isExistTableColumn($tableName, $field->name))
								{
									$originalFieldName = $field->name;
									break;
								}
		...
		if (count($data))
		{
			foreach ($data as $key => $fields)
			{
				...
				foreach ($fields as $subKey => $field)
				{
					...
					if (!$this->_objJSNJSDBUtils->isExistTableColumn($table, $subKey))
					{
		...
		if(count($data))
		{
			foreach ($data as $k1 => $fields)
			{
				...
				foreach ($fields as $k1 => $field)
				{
					...
					{
						if ($checkColumn && !$this->_objJSNJSDBUtils->isExistTableColumn($table, $value['change'])) continue;
			...
			foreach ($data as $k1 => $fields)
			{
				...
				foreach ($fields as $k1 => $field)
				{
					foreach ($field as $k2 => $value)
					{
						...
						if ($this->_objJSNJSDBUtils->isExistTableColumn($table, $k2))
						{
