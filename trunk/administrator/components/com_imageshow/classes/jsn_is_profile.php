 ...
 * @version $Id: jsn_is_profile.php 6648 2011-06-08 10:13:51Z giangnd $
...
class JSNISProfile
{
	...
	function checkProfileExist($name, $ignoreProfileID = 0)
	{
		$condition = '';

		if ($ignoreProfileID > 0)
		{
			$condition = ' AND configuration_id <> '.(int)$ignoreProfileID . ' ';
		}

		$query = 'SELECT * FROM #__imageshow_configuration WHERE BINARY configuration_title LIKE '.$this->_db->quote($name).$condition;
		$this->_db->setQuery($query);
	...
	{
		$query = 'SELECT COUNT(showlist_id) FROM #__imageshow_showlist WHERE configuration_id ='. (int)$profileID;
		$this->_db->setQuery($query);
