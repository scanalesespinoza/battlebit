 ...
 * @version $Id: default_inslangs.php 6505 2011-05-31 11:01:31Z trungnq $
					...
					foreach ($this->arrayFolder as $key=>$value)
					{
						if(isset($this->arrayFolder[$key]['site']) && $this->arrayFolder[$key]['site'] != 1){
							if ($this->arrayFolder[$key]['site'] == 2){
								$strErrorSite = $spanErrorOpen.' - '.JText::_('MAINTENANCE_LANG_CAN_NOT_BE_INSTALLED').$spanErrorClose.'&nbsp; <strong><span class="editlinktip hasTip" title="::'.JText::sprintf('MAINTENANCE_LANG_FOLDER_%S_MUST_HAVE_WRITABLE_PERMISSION', 'language/'.$key).'">[?]</span></strong>';
						...
						if(isset($this->arrayFolder[$key]['admin']) && $this->arrayFolder[$key]['admin'] != 1){
							if ($this->arrayFolder[$key]['admin'] == 2){
								$strErrorAdmin = $spanErrorOpen.' - '.JText::_('MAINTENANCE_LANG_CAN_NOT_BE_INSTALLED').$spanErrorClose.'&nbsp; <strong><span class="editlinktip hasTip" title="::'.JText::sprintf('MAINTENANCE_LANG_FOLDER_%S_MUST_HAVE_WRITABLE_PERMISSION', 'administrator/language/'.$key).'">[?]</span></strong>';
