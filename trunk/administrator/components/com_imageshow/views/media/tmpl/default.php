		...
		<tr>
			<td nowrap="nowrap">
				<form action="<?php echo JURI::base(); ?>index.php?option=com_imageshow&amp;controller=media&amp;task=upload&amp;tmpl=component&amp;<?php echo $this->session->getName().'='.$this->session->getId(); ?>&amp;pop_up=1&amp;<?php echo JUtility::getToken();?>=1" style="<?php echo ($act =='custom' || $act =='watermark' || $act =='showlist')? '': 'display:none;';?>" id="uploadForm" method="post" enctype="multipart/form-data">
						...
						<button type="submit" id="file-upload-submit" title="<?php echo JText::_('MEDIA_START_UPLOAD'); ?>"><?php echo JText::_('MEDIA_START_UPLOAD'); ?></button>
			...
			<td nowrap="nowrap">
				<form action="index.php" id="imageForm" method="post" enctype="multipart/form-data">
					<div style="float: left; <?php echo ($act =='custom' || $act =='watermark' || $act =='showlist')? '': 'display:none;';?>">
						...
						&nbsp;<button type="button" id="upbutton" title="<?php echo JText::_('MEDIA_DIRECTORY_UP') ?>"><?php echo JText::_('MEDIA_UP') ?></button>
					</div>
				</form>
			</td>
		</tr>
...
<form action="index.php" id="imageForm" method="post" enctype="multipart/form-data">
	...
	<fieldset class="jsn-image-url">
		<table border="0" width="100%" cellpadding="0" cellspacing="0">
			<tr>
				<td nowrap="nowrap"><label for="f_url"><?php echo JText::_('MEDIA_IMAGE_URL') ?>&nbsp;</label></td>
				<td width="100%"><input type="text" id="f_url" value="" readonly="readonly" /></td>
			</tr>
		</table>
	</fieldset>
    <div class="button">
	<button type="button" onclick="JSNISImageManager.onok('<?php echo $act?>');window.top.setTimeout('SqueezeBox.close()', 200);"><?php echo JText::_('MEDIA_SELECT') ?></button>
	<button type="button" onclick="window.top.setTimeout('SqueezeBox.close()', 200);"><?php echo JText::_('MEDIA_CANCEL') ?></button>
	</div>
