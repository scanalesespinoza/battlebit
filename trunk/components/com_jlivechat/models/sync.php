...
class JLiveChatModelSync extends JModel
{
    ...
    {
	if(ini_get('zlib.output_compression') == 'Off' || ini_get('zlib.output_compression') == 'off' || ini_get('zlib.output_compression') == 0 || !ini_get('zlib.output_compression'))
	{
	    $zlibOutputCompressionEnabled = false;
	}
	else
	{
	    $zlibOutputCompressionEnabled = true;
	}

	// Ensure gzip compression is enabled and zlib.output_compression is off since they cannot be used in conjunction with each other
	if($this->_settings->getSetting('use_gzip') == 1 && function_exists('gzfile') && !$zlibOutputCompressionEnabled)
	{
	    $compressedBuffer = ob_gzhandler($buffer, 5);

	    if(empty($compressedBuffer))
	    {
		// Browser doesn't support GZIP compression, send original
		echo $buffer;
	    }
	    else
	    {
		// Borwser supports gzip
		echo $compressedBuffer;
	    }
	}
	else
	{
