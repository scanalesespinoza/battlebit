...
var chatName = null;
var refreshInterval = 1500; // in milliseconds
...
var JLiveChat = {
    ...
    proactiveRecheckInterval: 10000, // In miliseconds, 10 seconds default
    memberId: null,
    chatSessionId: null,
    popupWindowObj: null,
	...
	if(typeof(window.jQuery) == 'undefined') {
	    ...
	    requiredJSLibs.push(JLiveChat.websiteRoot+'/components/com_jlivechat/js/jquery-1.6.3.min.js');
	}
	...
	if(JLiveChat.currentURI.indexOf("popup_mode") > -1) {
	    ...
	    setTimeout('JLiveChat.checkAutopopup();', 1500);
	}
    },
	...
	if(popupMode == 'iframe') {
	    ...
	    setTimeout('JLiveChat.monitorIFramePopup();', 300);
	} else {
	    // Open popup window
	    if(JLiveChat.iframeOpen) {
		JLiveChat.closeIFramePopup();
	...
	if(!isInIFrame) {
	    ...
	    window.parent.window.location.hash="#close_window";
	}
	...
	jQuery('#minimize-window-button').attr('onclick', '');
	jQuery('#minimize-window-button').click(function() {
	    JLiveChat.restoreIFramePopup();
	});

	window.parent.window.location.hash="#minimize_window";
	...
	jQuery('#minimize-window-button').attr('onclick', '');
	jQuery('#minimize-window-button').click(function() {
	    JLiveChat.minimizeWnd();
	});
	
	window.parent.window.location.hash="#restore_window";
	...
	if(String(document.location.href).indexOf('popup_mode=iframe') > -1 || String(document.location.href).indexOf('popup_mode/iframe') > -1) {
	    // This is in iframe window
	    isInIFrame = true;

	    try {
		if(!window.parent.JLiveChat.iframeOpen && window.location != window.parent.location) {
		    // iframe is no longer visible, so skip refresh
		    return false;
		}
	    } catch(err) {
